...
`src/lib/scraper/parse.ts` のセレクタ・抽出ロジックは公開情報や画面キャプチャから得た一般的な
ページ構造をもとにした**未検証の実装**である。実際に接続できる環境でHTML構造(view-source)を
確認し、必要に応じて調整すること。特に `parseVenueDays`(トップページの全24場の開催状況)は
場名テキストを起点にDOMを遡るヒューリスティックであり、実際のマークアップでの検証が必須。
